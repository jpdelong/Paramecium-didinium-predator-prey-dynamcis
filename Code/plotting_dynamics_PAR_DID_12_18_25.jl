...
gradient = cgrad(:seaborn_deep6,6,categorical = true) # create a color gradient
col_mix = gradient[1]
col_9 = gradient[2]
col_33 = gradient[3]
col_34 = gradient[4]
...
text!(ax_days_ext, (0.5, 20), text = "A", fontsize = 20)
text!(ax_cv, (0.5, 1.9), text = "B", fontsize = 20)

# call the figure names to look at them
f
f2
f3
...
f4 = Figure(size = (1500, 500))
    ax_pca_10 = Axis(f4[1, 1],ylabel="PCA2",title = "Up to day 10")
        xlims!(ax_pca_10,-2,2)
        ylims!(ax_pca_10,-3,3)
        text!(ax_pca_10, [(-1.9,2)], text = "A", fontsize = 18) # Annotate the empty subplot
    ax_pca_20 = Axis(f4[2, 1],ylabel="PCA2",title = "Up to day 20")
        xlims!(ax_pca_20,-2,2)
        ylims!(ax_pca_20,-3,3)
        text!(ax_pca_20, [(-1.9,2)], text = "B", fontsize = 18) # Annotate the empty subplot
    ax_pca_25 = Axis(f4[3, 1],ylabel="PCA2",xlabel="PCA1",title = "Up to day 24")
        xlims!(ax_pca_25,-2,2)
        ylims!(ax_pca_25,-3,3)
        text!(ax_pca_25, [(-1.9,2)], text = "C", fontsize = 18) # Annotate the empty subplot
    
    ax_pcade_10 = Axis(f4[1, 2],ylabel="Day of extinction",title = "Up to day 10",xticks=([-2,-1,0,1,2]))
        xlims!(ax_pcade_10,-2,2)
        ylims!(ax_pcade_10,0,30)
        text!(ax_pcade_10, [(-1.9,25)], text = "D", fontsize = 18) # Annotate the empty subplot
    ax_pcade_20 = Axis(f4[2, 2],ylabel="Day of extinction",title = "Up to day 20")
        xlims!(ax_pcade_20,-2,2)
        ylims!(ax_pcade_20,0,30)
        text!(ax_pcade_20, [(-1.9,25)], text = "E", fontsize = 18) # Annotate the empty subplot
    ax_pcade_25 = Axis(f4[3, 2],ylabel="Day of extinction",xlabel="PCA1",title = "Up to day 24")
        xlims!(ax_pcade_25,-2,2)
        ylims!(ax_pcade_25,0,30)
        text!(ax_pcade_25, [(-1.9,25)], text = "F", fontsize = 18) # Annotate the empty subplot

# restrict data we use to FRs with more than a minimum amount of data, ["Mixed","9","33","34","56","57"]
morphdata_10 = morphdata[findall(morphdata.Day_of .< 10),:]
    ...
    df_extinction[!,:PCA2] .= mix_end_two[:]
        extinct_yn = zeros(25,1)
        extinct_yn[day_ext_mix[:] .> 22] .= 1.0
    df_extinction[!,:ext_yn] .= extinct_yn[:]

    lm_1 = lm(@formula(Days_to_extinction ~ PCA1 * PCA2), df_extinction) # linear model to test for effect on days to extinction
    lm_1 = lm(@formula(Days_to_extinction ~ PCA1 + PCA2), df_extinction)

    scatter!(ax_pcade_10,mix_end_one[:],day_ext_mix,color=:black)
    scatter!(ax_pcade_10,mix_end_one[findall(day_ext_mix .> 21)],day_ext_mix[findall(day_ext_mix .> 21)],color=:white,strokecolor=:black,strokewidth=1)
...
morphdata_20 = morphdata[findall(morphdata.Day_of .< 20),:]
    ...
    scatter!(ax_pcade_20,mix_end_one[findall(day_ext_mix .> 21)],day_ext_mix[findall(day_ext_mix .> 21)],color=:white,strokecolor=:black,strokewidth=1)
...
# no need to restrict data for the full time series
    ...
    scatter!(ax_pcade_25,mix_end_one[findall(day_ext_mix .> 21)],day_ext_mix[findall(day_ext_mix .> 21)],color=:white,strokecolor=:black,strokewidth=1)
...
f5 = Figure(size = (500, 500))
    ...
    extinction_day = 24.0 # change this to 10, 20, or 24 to evaluate different time periods over the experiment
