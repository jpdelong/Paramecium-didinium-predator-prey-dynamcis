function plot_morph_dynamics(num_reps,morphdata,mix_dishes,p_ext_mix,trait_one,trait_two,axes_name,
    ...
    mix_end_one = zeros(num_reps)
    mix_end_two = zeros(num_reps)
    ...
    scatter!(axes_name,mix_end_one[findall(p_ext_mix .< 18)],mix_end_two[findall(p_ext_mix .< 18)],color=:black)
    scatter!(axes_name,mix_end_one[findall(p_ext_mix .> 17)],mix_end_two[findall(p_ext_mix .> 17)],color=:yellow)
